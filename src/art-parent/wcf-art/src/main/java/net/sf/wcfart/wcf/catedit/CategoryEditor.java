...
import net.sf.wcfart.wcf.changeorder.ChangeOrderUtils;
...
public class CategoryEditor extends NestableComponentSupport implements ChangeOrderModel, CategoryModelChangeListener {
  ...
  Element renderCategory(Category cat) {
    ...
    int index = 0;
    for (Iterator<Item> it = cat.getItems().iterator(); it.hasNext();) {
      Item item = it.next();
  ...
  Element renderItem(Item item, Category cat) {
    ...
    if (item.isMovable() && (N > 1 || cat.isEmptyAllowed())) {
      for (Category other : model.getCategories()) {
  ...
  /**
   * @see net.sf.wcfart.wcf.changeorder.ChangeOrderModel#mayMove(Object, Object)
   */
