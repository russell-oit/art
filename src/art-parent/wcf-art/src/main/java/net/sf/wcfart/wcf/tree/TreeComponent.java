...
import net.sf.wcfart.wcf.changeorder.ChangeOrderMgr;
import net.sf.wcfart.wcf.changeorder.ChangeOrderModel;
...
public class TreeComponent extends NestableComponentSupport {
   ...
   */
  public void setComparator(Comparator<? super Object> comp) {
    expanded = new TreeSet<>(comp);
  }

  /**
   * set the model to use
   * @param newModel null or a TreeModel. If the TreeModel implements the ChangeOrderModel,
  ...
  public void expandSelected(boolean expandSelected) {
    expanded.clear();
    for (Iterator<Object> it = selectionMgr.getSelectionModel().getSelection().iterator(); it.hasNext();) {
      Object node = it.next();
   ...
   * @see net.sf.wcfart.wcf.changeorder.ChangeOrderModel
   */
