...
public class TabbedHandler implements NodeHandler, RequestListener {
  ...
  private ArrayList<PanelChangeListener> panelChangeListeners = new ArrayList<>();
  ...
  protected void fireChanged(Element currentPanel) {
    if (panelChangeListeners.size() > 0) {
      ...
      List<PanelChangeListener> copy = new ArrayList<>(panelChangeListeners);
      for (PanelChangeListener pcl : copy){
		  pcl.panelChanged(event);
    }
  }
  }
