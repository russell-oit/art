...
import net.sf.wcfart.wcf.utils.SqlUtils;
...
public class SqlGenerator implements SqlExprVisitor {
  ...
  public SqlGenerator(Map<String, String> columnMap) {
  ...
  protected String getColumnName(SqlColumnConstraint scc) {
    ...
    String qname = columnMap.get(columnId);
    if (qname != null)
      return qname;
    return columnId;
  }
  ...
  public void genAndOr(SqlExprWithOperands exp, String keyword) {
    Iterator<SqlExpr> it = exp.getOperands().iterator();
    SqlExpr opd = it.next();
    sb.append("(");
