...
//	private static final String FILENAME_REGEX = "[a-zA-Z0-9]{1,200}\\.[a-zA-Z0-9]{1,10}";
	...
	private static final String TWO_DOT_FILENAME_REGEX = "[a-zA-Z0-9_\\-\\s]{1,100}(\\.[a-zA-Z0-9]{1,10}){1,2}";
	private static final Pattern TWO_DOT_FILENAME_PATTERN = Pattern.compile(TWO_DOT_FILENAME_REGEX);
	...
	/**
	 * Returns <code>true</code> if the given file name is suitable to be used
	 * for a file on the computer. Two dot extensions are valid.
	 *
	 * @param finalFilename the file name to check. Includes the extension.
	 * @return <code>true</code> if the given file name is suitable to be used
	 * for a file on the computer
	 */
	public static boolean isValidTwoDot(String finalFilename) {
		Matcher matcher = TWO_DOT_FILENAME_PATTERN.matcher(finalFilename);
		return matcher.matches();
	}
}
