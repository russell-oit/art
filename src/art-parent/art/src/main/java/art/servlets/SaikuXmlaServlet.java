...
public class SaikuXmlaServlet extends Olap4jXmlaServlet {
	...
	private static final Logger logger = LoggerFactory.getLogger(SaikuXmlaServlet.class);

	private static SaikuConnectionManager connectionManager;
	...
	public void init(ServletConfig config) throws ServletException {
		super.init(config);
		connectionManager = Config.getGlobalSaikuConnectionManager();
	}
		...
		return new ConnectionFactory() {
			...
			public OlapConnection getConnection(String s, String s1, String s2, Properties properties) throws SQLException {
				try {
					...
					if (s != null) {
						return connectionManager.getExistingOlapConnection(s);
					} else {
						for (Map.Entry<String, OlapConnection> entry : connectionManager.getCurrentOlapConnections().entrySet()) {
	...
	private static class SaikuXmlaExtraImpl extends XmlaHandler.XmlaExtraImpl {
		...
		public List<Map<String, Object>> getDataSources(
			...
			Map<String, OlapConnection> conns = connectionManager.getCurrentOlapConnections();
