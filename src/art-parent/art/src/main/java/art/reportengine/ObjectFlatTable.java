...
import net.sf.reportengine.components.AbstractColumnBasedTable;
...
/**
 * Modification of net.sf.reportengine.components.DefaultFlatTable to use
 ...
 *
 */
final class ObjectFlatTable extends AbstractColumnBasedTable implements FlatTable {

	/**
	 * the one and only logger
	 */
	private static final Logger LOGGER = LoggerFactory.getLogger(ObjectFlatTable.class);
	 ...
	 */
	ObjectFlatTable(TableInput input,
	...
	private AbstractAlgo configReportAlgo(final boolean hasBeenPreviouslySorted) {
		...
		if (getShowDataRows()) {
			reportAlgo.addMainStep(new ObjectDataRowsOutputStep());
		}
	...
	protected void validate() {
		LOGGER.debug("validating flat report");
	...
	public void output(AbstractReportOutput reportOutput) {
		...
		LOGGER.debug("flat table output ended with result {}", result);
	}
}
