	...
	public StepResult<Integer> execute(NewRowEvent newRowEvent, StepInput stepInput) {
		...
		for (int i = 0; i < finalReportGroupCount; i++) {
			...
			if (currentGrpCol.showDuplicates()
				...
				cellPropsBuilder = new CellProps.Builder(valueForCurrentColumn);
			} else {
		...
		StepResult<Integer> stepResult = new StepResult<>(StepIOKeys.DATA_ROW_COUNT, getDataRowCount(stepInput) + 1);
		return stepResult;
	}
