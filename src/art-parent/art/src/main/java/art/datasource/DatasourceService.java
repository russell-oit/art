...
public class DatasourceService {
	...
	public List<Datasource> getUnusedDatasources() throws SQLException {
		...
		String sql = "SELECT * FROM ART_DATABASES AD"
				+ " WHERE NOT EXISTS ("
				+ " SELECT 1 FROM ART_QUERIES WHERE DATASOURCE_ID = AD.DATABASE_ID"
				+ " UNION ALL"
				+ " SELECT 1 FROM ART_JOBS WHERE CACHED_DATASOURCE_ID = AD.DATABASE_ID"
				+ " UNION ALL"
				+ " SELECT 1 FROM ART_SETTINGS WHERE LOGS_DATASOURCE_ID = AD.DATABASE_ID"
				+ " )";
