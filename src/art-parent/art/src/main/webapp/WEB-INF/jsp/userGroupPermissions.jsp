<%-- 
    Document   : userGroupGroupPermissions
    Created on : 28-Jun-2018, 17:51:29
...
<spring:message code="page.title.userGroupPermissions" var="pageTitle"/>
				...
				tbl.find('tbody').on('click', '.deleteRecord', function () {
					...
					bootbox.confirm({
						...
						callback: function (result) {
							if (result) {
								//userGroup confirmed delete. make delete request
								$.ajax({
									type: "POST",
									dataType: "json",
									url: "${pageContext.request.contextPath}/deleteUserGroupPermission",
									data: {id: recordId},
									success: function (response) {
										if (response.success) {
											table.row(row).remove().draw(false); //draw(false) to prevent datatables from going back to page 1
											notifyActionSuccess("${permissionRemovedText}", recordName);
										} else {
			...
			<tbody>
				<c:forEach var="userGroupPermission" items="${userGroupPermissions}">
					<tr data-name="${encode:forHtmlAttribute(userGroupPermission.userGroup.name)} -
						...
						<td>${encode:forHtmlAttribute(userGroupPermission.permission.name)}</td>
						<td>
