<%-- 
    Document   : reportGroupMembership
    Created on : 12-Nov-2017, 12:47:18
...
<spring:message code="page.title.reportGroupMembership" var="pageTitle"/>
		...
		<table id="memberships" class="table table-striped table-bordered">
			...
			<tbody>
				<c:forEach var="membership" items="${memberships}">
					<tr data-name="${encode:forHtmlAttribute(membership.report.name)} -
						${encode:forHtmlAttribute(membership.reportGroup.name)}"
						data-id="${membership.report.reportId}-${membership.reportGroup.reportGroupId}">

						<td><encode:forHtmlContent value="${membership.report.name}"/></td>
						<td><encode:forHtmlContent value="${membership.reportGroup.name}"/></td>
						<td>
