...
/**
 * Produces a {@link org.jfree.data.general.Dataset} which will be rendered as a
 * chart afterwards.
 *
...
public interface DatasetProducer extends Serializable {
	...
	/**
	 * By default the the name of the JSP attribute holding the producer
	 * instance is passed to the produceDataset method as a parameter.
	 */
	public static final String PRODUCER_ATTRIBUTE_NAME = "net.sf.cewolfart.DatasetProducer.id";
	 ...
	 * produced data can be reused. If the data which had already been used for
	 * chart rendering is still valid this method should return
	 * <code>true</code>. If possible the Cewolf framework will try to reuse the
	 * rendered chart image. If this is not possible because of some
	 * circumstances (e.g. the chart had been removed from the image cache) the
	 * produceDataset method is called afterwards. Therefore there is no
	 * guarantee that the dataset production is always avoided if this method
	 * returns <code>true</code>.
	 *
	 * @param params the production parameters of the already produced data
	 * @param since the point in time when the already produced data had been
	 * produced
	 ...
	 * otherwise
	 */
	//https://sourceforge.net/p/cewolf/discussion/192228/thread/f5106f26/
	boolean hasExpired(Map<String, Object> params, Date since);
