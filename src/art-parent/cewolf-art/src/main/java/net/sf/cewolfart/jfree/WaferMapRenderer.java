...
public class WaferMapRenderer extends AbstractRenderer {
	...
	private int getPaintIndex(Number value) {
		return paintIndex.get(value).intValue();
	}
	...
	private void makePaintIndex() {
		...
		if (uniqueValues.size() <= this.paintLimit) {
			...
			for (Iterator i = uniqueValues.iterator(); i.hasNext();) {
				paintIndex.put((Number) i.next(), Integer.valueOf(count++));
			}
		} else {
	...
	private void makePositionIndex(Set uniqueValues) {
		...
		int paint = 0;
		for (Iterator i = uniqueValues.iterator(); i.hasNext();) {
			this.paintIndex.put((Number) i.next(), Integer.valueOf(paint));
	...
	private void makeValueIndex(Number max, Number min, Set uniqueValues) {
		...
		for (Iterator i = uniqueValues.iterator(); i.hasNext();) {
			...
			paintIndex.put(value, Integer.valueOf(paint));
		}
	}
			...
			if (paintIndex.size() <= this.paintLimit) {
				...
				for (Map.Entry<Number, Integer> entry : paintIndex.entrySet()) {
					if (unique.add(entry.getValue())) {
						String label = getMinPaintValue(entry.getValue()).toString()
								+ " - " + getMaxPaintValue(entry.getValue()).toString();
						String description = label;
						Shape shape = new Rectangle2D.Double(1d, 1d, 1d, 1d);
						Paint paint = getSeriesPaint(entry.getValue());
						Paint outlinePaint = Color.black;
						Stroke outlineStroke = DEFAULT_STROKE;
	...
	private Number getMinPaintValue(Integer index) {
		...
		for (Entry<Number, Integer> entry : paintIndex.entrySet()) {
