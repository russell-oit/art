...
/**
 * A postprocessor for altering the domain and range of a chart, essentially
 * creating a way to "zoom into" a chart. The zooming cabilities for the range
 * (y) axis works for all plots.<BR>
 * The capabilities for the domain (x) axis work only for x/y and timeseries
 * plots (currently only has capabilities to define hours) only, not category
 * plots.<BR>
 * (Note: in order for the set range capabilities to work properly, it is
 * recommended that the "rangeIncludesZero" be set to false using a
 * VisualEnhancer postprocessor.)<BR>
 * The following parameters can be used:
 * <BR><b>lowerRangeVal</b> optional, default value produced by chart; the
 * lowest value that will be graphed on the range (y) axis
 * <BR><b>upperRangeVal</b> optional; default value produced by chart; the
 * highest value that will be graphed on the range (y) axis
 * <BR><b>lowerDomainValN</b> optional; default value produced by chart; the
 * lowest value that will be graphed on the domain (x) axis for a numerical plot
 * (xy plot)
 * <BR><b>upperDomainValN</b> optional; default value produced by chart; the
 * highest value that will be graphed on the domain (x) axis for a numerical
 * plot (xy plot)
 * <BR><b>lowerDomainValD</b> optional; default value produced by chart; the
 * minimum time that will be graphed on the domain (x) axis for a time scale
 * graph (timeseries plot)
 * <BR><b>upperDomainValD</b> optional; default value produced by chart; the
 * maximum time that will be graphed on the domain (x) axis for a time scale
 * graph (timeseries plot)
 * <BR><b>zoomFactor</b> optional; default 1.0; value to zoom in on the center
 * point
 * <BR><b>anchorValueD</b> optional; no default value; the domain axis (X axis)
 * of xyplots is centered around this value
 * <BR><b>anchorValueR</b> optional; no default value; the range axis (Y axis)
 * of xyplots is centered around this value
 * <P>
 * Example usage:
 * <P>
...
public class ZoomBothAxis implements ChartPostProcessor, Serializable {
	...
	public void processChart(JFreeChart chart, Map<String, String> params) {
		...
		String str = params.get("lowerRangeVal");
		if (str != null) {
			try {
		...
		String strL = params.get("lowerDomainValD");
		String strU = params.get("upperDomainValD");
		if (strL != null && strU != null) {
			if (!strL.equals(strU)) {
				...
				int testerHour = lowerDomainValD.get(Calendar.HOUR_OF_DAY);
				int testerMinute = lowerDomainValD.get(Calendar.MINUTE);
				if (hour > testerHour) {
					lowerDomainValD.set(Calendar.HOUR_OF_DAY, hour);
					lowerDomainValD.set(Calendar.MINUTE, minute);
				} else if (hour == testerHour && minute > testerMinute) {
					lowerDomainValD.set(Calendar.HOUR_OF_DAY, hour);
					lowerDomainValD.set(Calendar.MINUTE, minute);
				}
				...
				testerHour = upperDomainValD.get(Calendar.HOUR_OF_DAY);
				testerMinute = upperDomainValD.get(Calendar.MINUTE);
				if (hour < testerHour) {
