...
public class WaferMapPlot extends Plot implements RendererChangeListener {
            ...
            for (int y = 1; y <= ychips; y++) {
				...
				if (showCellValues) {
					...
					if (chipValue != null) {
						...
						float xPos = (float) (bounds.getMinX() + ((bounds.getMaxX() - bounds.getMinX()) / 10.0f));
    ...
    public boolean equals(Object obj) {
        ...
        WaferMapPlot that = (WaferMapPlot) obj;
        if (!super.equals(obj)) {
            return false;
        }
        if (!ObjectUtilities.equal(this.orientation, that.orientation)) {
            return false;
        }
