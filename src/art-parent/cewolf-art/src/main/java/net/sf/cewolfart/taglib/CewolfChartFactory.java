...
import net.sf.cewolfart.jfree.XYConditionRenderer;
...
public abstract class CewolfChartFactory implements ChartConstants, AxisConstants, LayoutConstants {
  ...
  private static Map<String,CewolfChartFactory> factories = new HashMap<String,CewolfChartFactory>();
  ...
  private static final int getLayoutConstant (String layout) {
    ...
    switch (getChartTypeConstant(chartType)) {
      ...
      case VERTICAL_XY_BAR :
		...
		boolean dateAxis = data instanceof DynamicTimeSeriesCollection || data instanceof TimePeriodValuesCollection
							|| data instanceof TimeSeriesCollection || data instanceof TimeTableXYDataset;
        chart = ChartFactory.createXYBarChart(title, xAxisLabel, dateAxis, yAxisLabel,
	...
	case WAFER :
        ...
        WaferMapPlot wmPlot = new WaferMapPlot((WaferMapDataset) data);
        WaferMapRenderer wmRenderer = new WaferMapRenderer();
        wmPlot.setRenderer(wmRenderer);
		// the next 3 lines should not be necessary - something's not right in the WaferMapRenderer
		for (int i=0; i<((WaferMapDataset) data).getUniqueValueCount(); i++) {
			wmRenderer.lookupSeriesPaint(i);
		}
        chart = new JFreeChart(title, JFreeChart.DEFAULT_TITLE_FONT, wmPlot, showLegend);
		ChartFactory.getChartTheme().apply(chart);
		return chart;

	case HEATMAP :
		check(data, XYZDataset.class, chartType);
		XYPlot xyPlot = new XYPlot((XYZDataset) data, new NumberAxis(), new NumberAxis(), new XYBlockRenderer());
        chart = new JFreeChart(title, JFreeChart.DEFAULT_TITLE_FONT, xyPlot, showLegend);
		ChartFactory.getChartTheme().apply(chart);
		return chart;
    ...
    public static JFreeChart getOverlaidChartInstance (String chartType, String title, String xAxisLabel,
			String yAxisLabel, int xAxisType, int yAxisType, List<PlotDefinition> plotDefinitions, boolean showLegend)
    ...
    switch (chartTypeConst) {
      case OVERLAY_XY :
        ...
        // get main plot
        PlotDefinition mainPlotDef = plotDefinitions.get(0);
        ...
        for (int plotidx = 1;plotidx<plotDefinitions.size();plotidx++) {
          PlotDefinition subPlotDef = plotDefinitions.get(plotidx);
      ...
      case OVERLAY_CATEGORY ://added by lrh 2005-07-11
        ...
        // get main plot
        mainPlotDef = plotDefinitions.get(0);
  ...
  public static JFreeChart getCombinedChartInstance (String chartType, String title, String xAxisLabel,
  			String yAxisLabel, List<PlotDefinition> plotDefinitions, String layout, boolean showLegend)
    	throws ChartValidationException, DatasetProduceException {
    final int chartTypeConst = getChartTypeConstant(chartType);
	CewolfRenderer.chartUsed(chartType);
    switch (chartTypeConst) {
        ...
        switch (layoutConst) {
          case DOMAIN :
            ...
            for (int i = 0; i < plotDefinitions.size(); i++) {
              PlotDefinition pd = plotDefinitions.get(i);
